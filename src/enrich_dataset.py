...
def enrich_insurance_dataset(df):
    ...
    df_enriched['PROVINCE_AVG_INCOME'] = df_enriched['CLAIM_PROVINCE'].map(
        lambda x: provinces_income.get(x, 25000 + np.random.normal(0, 2000))
    )
    ...
    df_enriched['PROVINCE_ACCIDENT_RATE'] = df_enriched['CLAIM_PROVINCE'].map(
        lambda x: provinces_accident_rate.get(x, 8.0 + np.random.normal(0, 1.0))
    )
    ...
    weather_weights = [0.5, 0.2, 0.15, 0.07, 0.05, 0.03]
    ...
    df_enriched['VEHICLE_ESTIMATED_VALUE'] = df_enriched['VEHICLE_BRAND'].map(
        lambda x: brand_avg_value.get(x, 25000 + np.random.normal(0, 5000))
    )
    ...
    df_enriched['VEHICLE_POWER_HP'] = df_enriched['VEHICLE_BRAND'].map(
        lambda x: brand_avg_power.get(x, 120 + np.random.normal(0, 20))
    ).astype(int)
    ...
    df_enriched['VEHICLE_RISK_INDEX'] = (
        df_enriched['VEHICLE_POWER_HP'] / 100 + 
        df_enriched['PROVINCE_ACCIDENT_RATE'] / 10 +  
        np.random.normal(0, 0.5, size=len(df_enriched))
    ).clip(1, 10)
    ...
    df_enriched['CLAIM_FREQUENCY_RATIO'] = (
        df_enriched['PREVIOUS_CLAIMS_COUNT'] / df_enriched['CUSTOMER_TENURE_YEARS'].clip(1) 
    )
    
    # 3.3 Indice combinato rischio cliente-veicolo
    df_enriched['COMBINED_RISK_INDEX'] = (
        0.3 * df_enriched['CLAIM_FREQUENCY_RATIO'].clip(0, 5) / 5 +  
        0.4 * df_enriched['VEHICLE_RISK_INDEX'] / 10 +  
        0.2 * df_enriched['PROVINCE_ACCIDENT_RATE'] / 10 + 
        0.1 * (100 - df_enriched['POLICYHOLDER_AGE'].clip(18, 90)) / 82 
    ).clip(0, 1)
    ...
    df_enriched['CUSTOMER_VALUE_INDEX'] = (
        ...
        df_enriched['CUSTOMER_TENURE_YEARS'].clip(1) / 10 *  
        (1 - df_enriched['COMBINED_RISK_INDEX'] * 0.5) 
    ) / 1000
